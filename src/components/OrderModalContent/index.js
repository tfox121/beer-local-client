...
import { Modal, Header, Table, Button, Grid, Message } from 'semantic-ui-react';
  ...
  return (
    <Modal.Content style={{ overflowX: 'scroll' }}>
      ...
      <Table celled unstackable style={{ minWidth: '600px' }}>
        ...
        <Table.Body>
          {orderItems
            .filter((stockItem) => stockItem.orderQuant)
            .map((stockItem) => (
          ...
          <Table.Row>
            ...
            <Table.HeaderCell>
              <NumberFormat
                ...
                fixedDecimalScale
                prefix='£'
                value={orderItems.reduce((acc, val) => {
                  acc +=
                    val.orderChange !== 'delete' && val.price * val.orderQuant;
                  return acc;
                }, 0)}
              />
            </Table.HeaderCell>
