...
const minSpendPromoCopy = (value) => (
  <>
    If you spend at least <strong>£{Number(value).toFixed(2)}</strong>,
  </>
);
...
const moneyOffDiscountCopy = (value) => (
  <>
    {' '}
    you will get a <strong>£{Number(value).toFixed(2)}</strong> discount.
  </>
);
...
const percentageOffDiscountCopy = (percentage) => (
  <>
    {' '}
    you will get a <strong>{Number(percentage).toFixed(1)}%</strong> discount.
  </>
);
...
const promotionCopySelection = (promotionObj, availableStockArr) => {
  if (
    ...
    !availableStockArr.filter(
      (stockItem) => stockItem.key === promotionObj.multibuyProduct,
    ).length
  ) {
    return null;
  }
  if (
    promotionObj.multibuyType === 'packageType' &&
    !PACK_SIZES[promotionObj.multibuyPackageType]
  ) {
    return null;
  }
  if (
    ...
    !availableStockArr.filter(
      (stockItem) => stockItem.key === promotionObj.freeItemProduct,
    ).length
  ) {
    return null;
  }
  return (
    <>
      ...
      {promotionObj.discountType === 'freeItems' &&
        freeItemDiscountCopy(
          promotionObj.freeItemQuantity,
          availableStockArr.filter(
            (stockItem) => stockItem.key === promotionObj.freeItemProduct,
          )[0].text,
        )}
